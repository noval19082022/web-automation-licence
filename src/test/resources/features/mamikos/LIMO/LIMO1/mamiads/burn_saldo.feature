...
Feature: Mamiads Burn Saldo
  ...
  Scenario Outline: [MamiAds][Naikkan Iklan] Switch OFF ads while saldo burn > 0 from maximal budget
    ...
    Examples:
      | adsName                                | currentPosisiIklan | currentToggle | currentStatusDesc                                    | currentAnggaranDesc                                       | actionButton    | messageToast              | expectedPosisiIklan | expectedToggle | expectedStatusDesc              |
      | burn mamiads Purwokerto Timur Banyumas | naik               | on            | Posisi iklan telah naik di hasil pencarian properti. | Hari ini Rp1.500 sudah dipakai dari batas pemakaian saldo | Ya, Nonaktifkan | Iklan berhenti dinaikkan. | tidak-naik          | off            | Klik tombol untuk naikkan iklan |